...
6. *schedule* - View upcoming broadcast schedule`,
  ...
  if (trimmedText === "next matches") {
    ...
    if (upcomingFixtures.length === 0) {
      await sock.sendMessage(from, {
        text: "No upcoming matches at the moment.",
      });
      return;
    }
  ...
  if (trimmedText === "premium") {
    ...
    // Generate and send premium upgrade message with payment link
    const premiumMessage = generatePremiumMessage(user.email);
    await sock.sendMessage(from, {
      text: premiumMessage,
      linkPreview: true,
    });
...
async function handlePredictFlow(from: string, sock: any) {
  ...
  if (upcomingFixtures.length === 0) {
    await sock.sendMessage(from, {
      text: "No upcoming matches to predict on.",
    });
    return;
  }
  ...
  for (const fixture of upcomingFixtures) {
    ...
    const timeUntilKickoff =
      (fixture.kickoffTime.getTime() - now.getTime()) / (60 * 1000);
...
async function handleScheduleCommand(from: string, sock: any) {
  ...
  for (const fixture of allFixtures) {
    const broadcastTime = new Date(
      fixture.kickoffTime.getTime() - 3 * 60 * 60 * 1000,
    );
...
async function handleScorePrediction(
  ...
  if (isNaN(homeScore) || isNaN(awayScore)) {
    await sock.sendMessage(from, {
      text: "❌ Invalid score format. Use: *2-1*",
    });
    return;
  }
        ...
        for (const match of matchesDue) {
          ...
          for (const user of users) {
            try {
              ...
              console.error(
                `[v0] Failed to send broadcast to ${user.uniqueUsername}:`,
                error,
              );
            }
          }
